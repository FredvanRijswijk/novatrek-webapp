// Background service worker for NovaTrek Safari extension
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete' && tab.url) {
    ...
    if ((url.hostname === 'localhost' && url.port === '3000' && url.pathname === '/extension-auth') || 
        (url.hostname === 'novatrek.app' && url.pathname === '/extension-auth')) {
...
async function saveToNovaTrek(data: CaptureData) {
  try {
    ...
    if (response.ok) {
      ...
      // If token expired, clear storage and open popup
