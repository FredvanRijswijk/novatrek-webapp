...
import { GooglePlacesClientV2 } from '@/lib/google-places/client-v2';
...
export function useGooglePlacesV2(): UseGooglePlacesReturn {
  const [isLoading, setIsLoading] = useState(!isGoogleMapsLoaded());
  const [error, setError] = useState<string | null>(null);
  const [client, setClient] = useState<GooglePlacesClientV2 | null>(null);
  ...
  useEffect(() => {
    ...
    if (!isGoogleMapsLoaded()) {
      loadGoogleMapsAPI(GOOGLE_MAPS_API_KEY)
        .then(() => {
          setClient(new GooglePlacesClientV2());
          setIsLoading(false);
        })
        .catch((err) => {
      ...
      const destinations = await Promise.all(
        predictions.slice(0, 5).map(async (prediction) => {
          try {
            ...
            return GooglePlacesClientV2.placeToDestination(placeDetails);
          } catch (err) {
  ...
  const searchActivities = useCallback(async (
    ...
    try {
      const places = await client.searchActivities(location, query);
      return places.map(place => ({
        id: place.id,
        name: place.displayName?.text || place.displayName || '',
        type: mapPlaceTypeToActivityType(place.primaryType || ''),
        ...
        rating: place.rating,
        images: place.photos?.map(photo => photo.getURI({ maxWidth: 400, maxHeight: 300 })),
...
function mapPlaceTypeToActivityType(primaryType: string): string {
  ...
  return typeMap[primaryType] || 'activity';
}
