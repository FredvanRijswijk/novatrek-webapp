/**
 * Server-side Weather API client using Firebase Admin SDK
 */
...
export class WeatherServerClient {
  private static instance: WeatherServerClient
  private apiKey: string
  private cacheHours = 6 // Cache weather data for 6 hours

  private constructor() {
    this.apiKey = process.env.OPENWEATHER_API_KEY || process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY || ''
  }

  static getInstance(): WeatherServerClient {
    if (!WeatherServerClient.instance) {
      WeatherServerClient.instance = new WeatherServerClient()
    }
    return WeatherServerClient.instance
  }
  ...
  async getWeather(lat: number, lng: number, date: Date): Promise<WeatherData | null> {
    console.log('WeatherServerClient.getWeather called with:', { lat, lng, date, apiKey: !!this.apiKey })
    ...
    if (cached) {
      ...
      const weather = cached.forecast.find(w => 
        new Date(w.date).toISOString().split('T')[0] === targetDate
      )
  ...
  /**
   * Get cached weather data using Admin SDK
   */
  private async getCachedWeather(locationKey: string): Promise<WeatherCache | null> {
    try {
      ...
      const cacheDoc = await adminDb.collection('weatherCache').doc(locationKey).get()
  ...
  /**
   * Fetch weather from API and cache it using Admin SDK
   */
  private async fetchAndCacheWeather(
    ...
    try {
      ...
      // Cache the data using Admin SDK
      const cacheData: any = {
        locationKey,
      ...
      const adminDb = getAdminDb()
      if (!adminDb) {
        console.error('Firebase Admin SDK not initialized - cannot cache weather data')
        // Return data anyway even if we can't cache it
      } else {
        await adminDb.collection('weatherCache').doc(locationKey).set(cacheData)
      }
